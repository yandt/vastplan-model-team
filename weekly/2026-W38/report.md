...
- 评分口径：口径按权重（41% 效果 + 34% 覆盖 + 25% 独立；花费/时间/token 三项资源轴不进综合分，只作参考列）在同活动类型内 min-max 归一到 0～100 后加权：效果（均质量）、覆盖（同名问题去重后的重要性加权成立数 ÷ 所参与轮次全场去重合计）、独立（独有（重要性加权）/ 成立（重要性加权））、时间（每次分钟，越快越好）、token（每条成立 token，越省越好）；花费（每次花费）、时间（每次分钟）、token（每条成立 token）三项资源轴与效率分一样只作参考列，不进综合分；某项没采集到时（如订阅制模型没有金额）该轴按中位 50 记，并在评论里标注。效率分（成立重要性÷耗时）只作参考列，不进综合分。
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | DeepSeek V4.1 Flash（max） | max | 97.0 | 25/19 | 7.4/8.2/-0.8 | 53/46 | 21 | 15 | 78% | 17.38/6.61 | 1.05 |
| 2 | DeepSeek V4.1 Flash（high） | high | 93.7 | 11/0 | 6.5/0.0/+6.5 | 24/0 | 10 | 8 | 75% | 1.07/0.00 | 1.17 |
| 3 | Muse Spark 1.3 Contributor | xhigh | 74.6 | 28/14 | 4.8/5.2/-0.5 | 58/22 | 21 | 31 | 65% | 0.33/0.12 | 3.05 |
| 4 | K3 方舟 Agent Plan | max | 69.5 | 28/29 | 4.9/5.9/-1.0 | 49/49 | 15 | 15 | 77% | 16.51/16.31 | 0.72 |
| 5 | Grok 4.6 Extra High | xhigh | 67.0 | 25/40 | 4.8/7.2/-2.4 | 38/70 | 9 | 11 | 78% | 33.89/45.96 | 0.72 |
| 6 | GLM-5.3 | max | 66.3 | 28/43 | 4.5/5.3/-0.8 | 43/73 | 13 | 14 | 75% | 12.16/17.91 | 0.84 |
| 7 | GLM-5.3-Flash | max | 64.3 | 28/40 | 4.9/5.7/-0.8 | 45/59 | 12 | 9 | 83% | 0.91/1.05 | 0.50 |
| 8 | DeepSeek V4 Flash | max | 62.3 | 23/35 | 4.5/3.8/+0.7 | 37/41 | 8 | 11 | 77% | 6.08/6.53 | 0.60 |
| 9 | MiMo V2.5 Pro | max | 24.5 | 28/35 | -0.7/0.7/-1.4 | 14/18 | 5 | 24 | 37% | 0.93/1.11 | 0.66 |
| 10※ | Gemini 3.8 Flash | high | 18.6 | 3/38 | 1.7/6.0/-4.3 | 2/77 | 0 | 1 | 67% | 3.74/32.67 | 0.32 |

※ 本期样本 < 5 场，结论打折；主力（≥5 场）第一：DeepSeek V4.1 Flash（max）（综合 97.0）
...
1. **DeepSeek V4.1 Flash（max）**（综合 97.0）— 长处：产出最高（均质量 7.4）、抓得最全（重要性加权占参与轮次 36%）、缓存命中最高（98%）；短处：时间相对最弱（每次 10.2 分钟）
2. **DeepSeek V4.1 Flash（high）**（综合 93.7）— 长处：独立相对最好（独有占比 40%）；短处：无突出短板
3. **Muse Spark 1.3 Contributor**（综合 74.6）— 长处：效率分最高（成立重要性÷耗时 3.05）、每条成立最便宜（$0.01）、最快（每次 2.5 分钟，档位 xhigh）、每条成立最省 token（306 千）；短处：效果相对最弱（均质量 4.8）
4. **K3 方舟 Agent Plan**（综合 69.5）— 长处：token相对最好（每条成立 464 千）；短处：时间相对最弱（每次 9.4 分钟）
5. **Grok 4.6 Extra High**（综合 67.0）— 长处：各项居中（均质量 4.8、准确率 78%）；短处：花费相对最弱（每次 $1.36）
6. **GLM-5.3**（综合 66.3）— 长处：token相对最好（每条成立 741 千）；短处：无突出短板
7. **GLM-5.3-Flash**（综合 64.3）— 长处：最准（83%）；短处：最慢（每次 11.0 分钟，档位 max）
8. **DeepSeek V4 Flash**（综合 62.3）— 长处：各项居中（均质量 4.5、准确率 77%）；短处：时间相对最弱（每次 9.9 分钟）
9. **MiMo V2.5 Pro**（综合 24.5）— 长处：花费相对最好（每次 $0.03）；短处：假阳最多（每次 0.86 条，准确率 37%）、均质量垫底（-0.7）
10. **Gemini 3.8 Flash**（综合 18.6）— 长处：各项居中（均质量 1.7、准确率 67%）；短处：每条成立最贵（$1.87）、每条成立最耗 token（11,881 千）、样本少（3 场，结论打折）
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | K3 方舟 Agent Plan | max | 96.9 | 18/18 | 23.7/23.9/-0.3 | 55/55 | 31 | 7 | 89% | 1.53/1.29 | 4.53/4.69 |
| 2 | Grok 4.6 Extra High | xhigh | 92.1 | 16/20 | 23.8/22.1/+1.7 | 49/56 | 25 | 4 | 92% | 5.55/5.97 | 4.47/4.33 |
| 3※ | Gemini 3.8 Flash | high | 61.6 | 3/20 | 18.0/17.8/+0.2 | 6/45 | 0 | 0 | 100% | 0.09/0.93 | 4.67/4.00 |
| 4 | GLM-5.3 | max | 57.9 | 17/21 | 17.1/16.6/+0.5 | 34/46 | 10 | 4 | 89% | 0.88/0.86 | 4.38/3.79 |
| 5 | Muse Spark 1.3 Contributor | xhigh | 55.9 | 17/7 | 16.4/10.1/+6.3 | 31/4 | 10 | 1 | 97% | 0.04/0.02 | 4.33/3.71 |
| 6 | DeepSeek V4.1 Flash（max） | max | 52.0 | 17/9 | 16.1/12.2/+3.9 | 32/11 | 8 | 2 | 94% | 0.65/0.24 | 4.40/3.33 |
| 7 | GLM-5.3-Flash | max | 49.5 | 17/21 | 15.5/15.2/+0.3 | 31/42 | 7 | 3 | 91% | 0.05/0.05 | 4.20/3.88 |
| 8 | DeepSeek V4 Flash | max | 48.4 | 16/21 | 14.9/15.2/-0.3 | 30/43 | 7 | 2 | 94% | 0.27/0.26 | 3.92/4.07 |
| 9 | MiMo V2.5 Pro | max | 35.2 | 16/18 | 10.7/9.1/+1.6 | 20/24 | 4 | 10 | 67% | 0.08/0.10 | 3.60/2.81 |
| 10※ | DeepSeek V4.1 Flash（high） | high | 0.0 | 2/0 | 6.0/0.0/+6.0 | 0/0 | 0 | 0 | 0% | 0.02/0.00 | 3.00/0.00 |

※ 本期样本 < 5 场，结论打折；主力（≥5 场）第一：K3 方舟 Agent Plan（综合 96.9）
...
1. **K3 方舟 Agent Plan**（综合 96.9）— 长处：每条成立最省 token（5 千）；短处：时间相对最弱（每次 1.7 分钟）
2. **Grok 4.6 Extra High**（综合 92.1）— 长处：产出最高（均质量 23.8）；短处：每条成立最贵（$0.11）、每条成立最耗 token（106 千）
3. **Gemini 3.8 Flash**（综合 61.6）— 长处：最准（100%）、抓得最全（重要性加权占参与轮次 47%）、方向契合最高（4.67）；短处：独立相对最弱（独有占比 0%）、样本少（3 场，结论打折）
4. **GLM-5.3**（综合 57.9）— 长处：token相对最好（每条成立 11 千）；短处：时间相对最弱（每次 3.2 分钟）
5. **Muse Spark 1.3 Contributor**（综合 55.9）— 长处：每条成立最便宜（$0.00）；短处：覆盖相对最弱（占参与轮次 25%）
6. **DeepSeek V4.1 Flash（max）**（综合 52.0）— 长处：效率分最高（成立重要性÷耗时 15.50）；短处：独立相对最弱（独有占比 23%）
7. **GLM-5.3-Flash**（综合 49.5）— 长处：花费相对最好（每次 $0.00）；短处：最慢（每次 3.4 分钟，档位 max）
8. **DeepSeek V4 Flash**（综合 48.4）— 长处：花费相对最好（每次 $0.02）；短处：时间相对最弱（每次 3.2 分钟）
9. **MiMo V2.5 Pro**（综合 35.2）— 长处：缓存命中最高（91%）；短处：效果相对最弱（均质量 10.7）
10. **DeepSeek V4.1 Flash（high）**（综合 0.0）— 长处：最快（每次 0.5 分钟，档位 high）；短处：假阳最多（每次 0.00 条，准确率 0%）、均质量垫底（6.0）、契合最低（3.00）、样本少（2 场，结论打折）
